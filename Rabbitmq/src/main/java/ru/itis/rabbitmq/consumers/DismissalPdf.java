...
public class DismissalPdf {
    public static void generate(UserDto user) {
        try {
            PdfDocument pdf = new PdfDocument(new PdfWriter("dismissal.pdf"));
            ...
            Paragraph p2 = new Paragraph("ООО Сбербанк");
            p2.setFixedPosition(400, 780, 100);
            document.add(p2);

            Paragraph p3 = new Paragraph("И. Мухаметгатину");
            p3.setFixedPosition(400, 760, 150);
            document.add(p3);
            ...
            Paragraph p6 = new Paragraph("Я, " + user.getFirst_name() + " " + user.getLast_name() + ", " + user.getNumberOfpassport() + ", " + user.getAge() + ", " + "прошу уволить меня по собственному желанию.");
